...
function StatCard({
  ...
  return (
    <div
      className={`bg-white dark:bg-pri border rounded-xl p-5 flex flex-col justify-between shadow-sm transition-colors ${
        alert ? "border-red-500/30 bg-red-500/5" : "border-gray-200 dark:border-zinc-800/80"
      }`}
    >
      <p className={`text-xs font-medium mb-3 ${alert ? "text-red-500" : "text-gray-500 dark:text-zinc-400"}`}>
        {label}
      </p>
      <p
        className={`text-2xl font-medium tracking-tight ${
          alert ? "text-red-600 dark:text-red-400" : "text-gray-900 dark:text-zinc-50"
        }`}
      >
        {value}
      </p>
      <p className={`text-[11px] mt-2 ${alert ? "text-red-500/70" : "text-gray-400 dark:text-zinc-500"}`}>
        {sub}
      </p>
    </div>
  );
...
function StatusBadge({ status }: { status: UserStatus }) {
  if (status === "active") {
    return (
      <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md border border-gray-200 dark:border-zinc-700 bg-gray-100 dark:bg-zinc-800 text-[10px] font-medium text-gray-700 dark:text-zinc-300">
        Active
      </span>
    );
  }
  return (
    <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md border border-red-200 dark:border-red-500/20 bg-red-50 dark:bg-red-500/10 text-[10px] font-medium text-red-600 dark:text-red-400 capitalize">
    ...
    <div className="p-4 sm:px-6 flex flex-col sm:flex-row sm:items-center justify-between gap-4 animate-pulse">
      <div className="flex items-center gap-4 min-w-0 flex-1">
        ...
        <div className="space-y-1.5">
          ...
          <div className="h-3 w-40 bg-gray-200 dark:bg-zinc-800 rounded" />
        </div>
      </div>
...
function UserModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/60 transition-opacity">
      <div className="relative bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl w-full max-w-md shadow-2xl flex flex-col overflow-hidden">
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-200 dark:border-zinc-800 bg-gray-50 dark:bg-pri">
          <div className="flex items-center gap-3">
            <div className="w-8 h-8 rounded-lg bg-gray-100 dark:bg-zinc-800/50 border border-gray-200 dark:border-zinc-700/50 text-gray-600 dark:text-zinc-400 flex items-center justify-center">
        ...
        <div className="p-6 space-y-5 overflow-y-auto custom-scrollbar">
          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-zinc-400 mb-1.5">Full Name</label>
            <input
              type="text"
              value={name}
              onChange={(e) => setName(e.target.value)}
              placeholder="e.g. John Doe"
              className="w-full bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg px-3 py-2.5 text-sm text-gray-900 dark:text-zinc-200 placeholder-gray-400 dark:placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-gray-400 dark:focus:ring-zinc-600 transition-all font-medium"
            />
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg px-3 py-2.5 text-sm text-gray-900 dark:text-zinc-200 placeholder-gray-400 dark:placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-gray-400 dark:focus:ring-zinc-600 transition-all"
            />
          </div>

          <div>
            <label className="block text-xs font-medium text-gray-500 dark:text-zinc-400 mb-1.5">Department</label>
            <select
              value={departmentId}
              onChange={(e) => setDepartmentId(e.target.value)}
              disabled={isLoadingDepartments}
              className="w-full bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg px-3 py-2.5 text-sm text-gray-900 dark:text-zinc-200 focus:outline-none focus:ring-1 focus:ring-gray-400 dark:focus:ring-zinc-600 transition-all appearance-none cursor-pointer disabled:opacity-60"
            >
            ...
            <p className="text-[11px] text-gray-500 dark:text-zinc-500 mt-1.5">
        ...
        <div className="p-4 border-t border-gray-200 dark:border-zinc-800 flex justify-end gap-3 bg-gray-50 dark:bg-zinc-900/80">
          <button
            onClick={onClose}
            className="px-4 py-2 rounded-lg text-sm text-gray-600 dark:text-zinc-400 font-medium hover:text-gray-900 dark:hover:text-zinc-100 hover:bg-gray-200 dark:hover:bg-zinc-800 transition-all"
          >
            Cancel
          </button>
          <button
            onClick={handleSave}
            disabled={!canSave}
            className="px-5 py-2 rounded-lg bg-gray-900 dark:bg-zinc-100 text-white dark:text-zinc-900 text-sm font-medium hover:bg-black dark:hover:bg-white transition-all shadow-sm disabled:opacity-50"
          >
...
function ActionModal({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 backdrop-blur-sm bg-black/60 transition-opacity">
      <div className="bg-white dark:bg-zinc-900 border border-gray-200 dark:border-zinc-800 rounded-xl w-full max-w-sm shadow-2xl overflow-hidden">
        <div className="p-6 space-y-4">
          <div
            className={`w-10 h-10 rounded-full flex items-center justify-center ${
              isDestructive
                ? "bg-red-50 dark:bg-red-500/10 border border-red-100 dark:border-red-500/20 text-red-500"
                : "bg-gray-100 dark:bg-zinc-800/50 border border-gray-200 dark:border-zinc-700/50 text-gray-600 dark:text-zinc-400"
            }`}
          >
            {icon}
          </div>
          <div>
            ...
            <p className="text-xs text-gray-600 dark:text-zinc-400 mt-1.5 leading-relaxed">{description}</p>
          </div>
        </div>
        <div className="px-6 pb-5 flex gap-3 justify-end bg-gray-50 dark:bg-zinc-900/80 pt-4 border-t border-gray-200 dark:border-zinc-800">
          <button
            onClick={onClose}
            disabled={isLoading}
            className="px-4 py-2 rounded-lg text-sm text-gray-600 dark:text-zinc-400 font-medium hover:text-gray-900 dark:hover:text-zinc-100 hover:bg-gray-200 dark:hover:bg-zinc-800 transition-all disabled:opacity-50"
          >
            Cancel
          </button>
          <button
            onClick={onConfirm}
            disabled={isLoading}
            className={`px-5 py-2 rounded-lg text-sm font-medium transition-all shadow-sm disabled:opacity-60 ${
              isDestructive
                ? "bg-red-600 text-white hover:bg-red-500"
                : "bg-gray-900 dark:bg-zinc-100 text-white dark:text-zinc-900 hover:bg-black dark:hover:bg-white"
            }`}
          >
      ...
      <div className="min-h-screen bg-gray-50 dark:bg-zinc-950 text-gray-900 dark:text-zinc-300 font-sans pb-16 selection:bg-gray-200 selection:text-gray-900 dark:selection:bg-zinc-800 dark:selection:text-zinc-100">
        ...
        <div className="border-b border-gray-200 dark:border-zinc-800/80 bg-white/80 dark:bg-zinc-950/80 backdrop-blur-md sticky top-0 z-10">
          <div className="px-6 h-16 flex items-center justify-between max-w-7xl mx-auto">
            <div className="flex items-center gap-3">
              ...
              <h1 className="text-sm font-medium text-gray-900 dark:text-zinc-100">Users</h1>
            ...
            <button
              ...
              className="flex items-center gap-1.5 bg-gray-900 dark:bg-zinc-100 hover:bg-black dark:hover:bg-white transition-all text-white dark:text-zinc-900 text-sm font-medium px-4 py-2 rounded-lg shadow-sm"
            >
        ...
        <div className="px-6 py-8 max-w-7xl mx-auto space-y-8">
          ...
          <div className="bg-white dark:bg-zinc-900/40 border border-gray-200 dark:border-zinc-800/50 rounded-xl overflow-hidden shadow-sm">
            {/* Toolbar */}
            <div className="p-4 border-b border-gray-200 dark:border-zinc-800/80 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50 dark:bg-pri">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <select
                  ...
                  className="w-full sm:w-auto bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg px-3 py-2 text-xs font-medium text-gray-700 dark:text-zinc-300 focus:outline-none focus:ring-1 focus:ring-gray-400 dark:focus:ring-zinc-600 appearance-none cursor-pointer"
                >
            ...
            {isError ? (
              <div className="py-24 flex flex-col items-center justify-center text-center">
                ...
                <p className="text-sm font-medium text-red-600 dark:text-red-400">Failed to load users</p>
              ...
              <div className="py-24 flex flex-col items-center justify-center text-center">
                <ShieldTick size={32} className="text-gray-300 dark:text-zinc-700 mb-4" color="currentColor" />
                <p className="text-sm font-medium text-gray-900 dark:text-zinc-300">No users found</p>
                <p className="text-xs text-gray-500 dark:text-zinc-500 mt-1">Adjust filters or search query.</p>
              </div>
            ) : (
              <div
                className={`divide-y divide-gray-100 dark:divide-zinc-800/80 transition-opacity ${
              ...
              >
                {users.map((user) => (
                  <div
                    key={user.id}
                    className="group p-4 sm:px-6 hover:bg-gray-50 dark:hover:bg-zinc-800/40 transition-colors flex flex-col sm:flex-row sm:items-center justify-between gap-4"
                  >
                    {/* User Info */}
                    <div className="flex items-center gap-4 min-w-0 flex-1">
                      <div className="w-10 h-10 rounded-full bg-gray-100 dark:bg-zinc-800 border border-gray-200 dark:border-zinc-700 flex items-center justify-center text-xs font-bold text-gray-500 dark:text-zinc-400 flex-shrink-0 uppercase tracking-widest">
                        {user.name
                          .split(" ")
                          .map((n) => n[0])
                          .join("")
                    ...
                    <div className="flex items-center gap-6 sm:gap-10">
                      <div className="hidden md:block text-left w-36">
                        <p className="text-[10px] font-medium text-gray-400 dark:text-zinc-500 uppercase tracking-widest mb-1">
                          Department
                        </p>
                      ...
                      {/* Actions */}
                      <div className="flex items-center gap-1.5 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity flex-shrink-0 pl-2 sm:border-l border-gray-200 dark:border-zinc-800">
                        <button
                          title="Edit User"
                          onClick={() => setEditingUser(user)}
                          className="p-1.5 rounded-md text-gray-500 dark:text-zinc-400 hover:bg-gray-200 dark:hover:bg-zinc-700 hover:text-gray-900 dark:hover:text-zinc-100 transition-colors"
                        >
                        ...
                        {user.status === "active" ? (
                          <button
                            ...
                            className="p-1.5 rounded-md text-gray-500 dark:text-zinc-400 hover:bg-red-50 dark:hover:bg-red-500/10 hover:text-red-600 dark:hover:text-red-400 transition-colors"
                          >
            ...
            {meta && meta.total_pages > 1 && (
              <div className="p-4 border-t border-gray-200 dark:border-zinc-800/80 flex items-center justify-between bg-gray-50 dark:bg-pri">
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="text-xs font-medium px-3 py-1.5 rounded-lg border border-gray-200 dark:border-zinc-800 text-gray-600 dark:text-zinc-400 disabled:opacity-40 disabled:cursor-not-allowed hover:bg-gray-100 dark:hover:bg-zinc-800 transition-colors"
                  >
                    Previous
                  </button>
                  <button
