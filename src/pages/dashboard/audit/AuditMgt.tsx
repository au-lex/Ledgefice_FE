    ...
    >
      <p
        className={`text-xs font-medium mb-3 ${
          alert ? "text-red-500" : "text-gray-500 dark:text-zinc-400"
        }`}
      >
        {label}
      </p>
      ...
      <p
        className={`text-[11px] mt-2 ${
          alert ? "text-red-500/70" : "text-gray-400 dark:text-zinc-500"
        }`}
      >
        {sub}
      </p>
...
function ActionBadge({ action }: { action: AuditAction }) {
  ...
  if (meta.isAlert) {
    return (
      <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md border border-red-200 dark:border-red-500/20 bg-red-50 dark:bg-red-500/10 text-[10px] font-medium text-red-600 dark:text-red-400 uppercase tracking-widest">
        {meta.icon}
        {meta.label}
      </span>
    );
  ...
  return (
    <span className="inline-flex items-center gap-1.5 px-2 py-1 rounded-md border border-gray-200 dark:border-zinc-700 bg-gray-100 dark:bg-zinc-800 text-[10px] font-medium text-gray-700 dark:text-zinc-300 uppercase tracking-widest">
      {meta.icon}
      {meta.label}
    </span>
  );
...
function LogRow({ log }: { log: AuditLog }) {
  ...
  return (
    <div className="p-4 sm:px-6 hover:bg-gray-50 dark:hover:bg-zinc-800/40 transition-colors flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-8">
      {/* Left: Time & Module Icon */}
      <div className="flex items-center gap-4 lg:w-48 flex-shrink-0">
        <div className="w-8 h-8 rounded bg-gray-100 dark:bg-zinc-800 flex items-center justify-center text-gray-500 dark:text-zinc-400 border border-gray-200 dark:border-zinc-700 shadow-sm flex-shrink-0">
          {moduleMeta?.icon}
        </div>
        <div>
          <p className="text-xs font-medium text-gray-900 dark:text-zinc-200 whitespace-nowrap">
            {formatted[0]}
          </p>
          <p className="text-[10px] text-gray-500 dark:text-zinc-500 font-mono mt-0.5">
            {formatted[1]}
          </p>
        </div>
      </div>
      ...
      <div className="flex-1 min-w-0 flex flex-col justify-center">
        <div className="flex items-center gap-2 mb-1.5 flex-wrap">
          ...
          <span className="text-[11px] font-mono text-gray-500 dark:text-zinc-400 bg-gray-100 dark:bg-zinc-900 px-1.5 py-0.5 rounded border border-gray-200 dark:border-zinc-800">
            {log.resource_id}
          </span>
        ...
        <p className="text-sm text-gray-600 dark:text-zinc-300 leading-relaxed line-clamp-2">
          {log.description}
        </p>
      </div>

      {/* Right: Technical Metadata (IP / Device) */}
      <div className="lg:w-48 flex-shrink-0 flex flex-row lg:flex-col gap-4 lg:gap-1 text-left lg:text-right border-t border-gray-100 dark:border-zinc-800 lg:border-t-0 pt-3 lg:pt-0 mt-2 lg:mt-0">
        <div className="flex-1 lg:flex-none">
          <p className="text-[9px] font-medium text-gray-400 dark:text-zinc-500 uppercase tracking-widest mb-0.5">
            IP Address
          </p>
          <p className="text-[11px] text-gray-700 dark:text-zinc-300 font-mono">
            {log.ip_address}
          </p>
        </div>
        <div className="flex-1 lg:flex-none">
          ...
          <p
            className="text-[11px] text-gray-600 dark:text-zinc-400 truncate"
...
function SkeletonRow() {
  return (
    <div className="p-4 sm:px-6 flex flex-col lg:flex-row lg:items-center gap-4 lg:gap-8 animate-pulse">
      <div className="flex items-center gap-4 lg:w-48 flex-shrink-0">
        <div className="w-8 h-8 rounded bg-gray-200 dark:bg-zinc-800" />
      ...
      <div className="lg:w-48 flex-shrink-0 space-y-2">
        ...
        <div className="h-2.5 w-20 bg-gray-200 dark:bg-zinc-800 rounded" />
      </div>
    </div>
  );
}
    ...
    <Layout>
      <div className="min-h-screen bg-gray-50 dark:bg-zinc-950 text-gray-900 dark:text-zinc-300 font-sans pb-16 selection:bg-gray-200 selection:text-gray-900 dark:selection:bg-zinc-800 dark:selection:text-zinc-100">
        ...
        <div className="px-6 py-8 max-w-7xl mx-auto space-y-8">
          ...
          <div className="bg-white dark:bg-zinc-900/40 border border-gray-200 dark:border-zinc-800/50 rounded-xl overflow-hidden shadow-sm flex flex-col">
            {/* Toolbar */}
            <div className="p-4 border-b border-gray-200 dark:border-zinc-800/80 flex flex-col sm:flex-row items-center justify-between gap-4 bg-gray-50 dark:bg-pri">
              ...
              <div className="flex items-center gap-3 w-full sm:w-auto">
                <div className="relative flex items-center bg-white dark:bg-zinc-950 border border-gray-200 dark:border-zinc-800 rounded-lg px-3 py-2 focus-within:ring-1 focus-within:ring-gray-400 dark:focus-within:ring-zinc-600 transition-all w-full sm:w-auto">
                  <Filter size={14} className="text-gray-400 dark:text-zinc-500 mr-2" color="currentColor" />
                  <select
                    value={moduleFilter}
                    onChange={(e) =>
                      resetToFirstPage(setModuleFilter)(e.target.value as AuditModule | "all")
                    }
                    className="bg-transparent text-xs font-medium text-gray-700 dark:text-zinc-300 focus:outline-none appearance-none cursor-pointer pr-4 w-full"
                  >
            ...
            {/* List */}
            {isError ? (
              <div className="py-24 flex flex-col items-center justify-center text-center bg-gray-50/50 dark:bg-zinc-900/20">
              ...
              <div className="py-24 flex flex-col items-center justify-center text-center bg-gray-50/50 dark:bg-zinc-900/20">
                <Activity size={32} className="text-gray-300 dark:text-zinc-700 mb-4" color="currentColor" />
                <p className="text-sm font-medium text-gray-900 dark:text-zinc-300">No logs found</p>
                <p className="text-xs text-gray-500 dark:text-zinc-500 mt-1">
