...
function FieldValueDisplay({ fv }: { fv: VoucherFieldValue }) {
  ...
  if (type === "date" && fv.value) {
    return <span className="text-sm text-zinc-200">{formatDate(fv.value)}</span>;
  }

  return <span className="text-sm text-zinc-200 break-words">{fv.value || "—"}</span>;
}
...
function StatCard({ label, value, sub, alert }: { label: string; value: string | number; sub: string; alert?: boolean }) {
  return (
    <div
      className={`border rounded-xl p-4 sm:p-5 flex flex-col justify-between shadow-sm min-w-0 transition-colors ${
        alert ? "border-red-500/30 bg-red-500/5" : "border-zinc-800/80 bg-pri"
      }`}
    >
      <p className={`text-xs font-medium mb-3 truncate ${alert ? "text-red-500" : "text-zinc-400"}`}>{label}</p>
      <p className={`text-xl sm:text-2xl font-medium tracking-tight ${alert ? "text-red-400" : "text-zinc-50"}`}>
        {value}
      </p>
...
function ReviewModal({ voucher, onClose, onApprove, onReject, onDismissDuplicate, isPending }: ReviewModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 backdrop-blur-sm bg-black/60 transition-opacity">
      <div className="relative bg-zinc-900 border border-zinc-800 rounded-xl w-full max-w-3xl shadow-2xl flex flex-col max-h-[92vh] overflow-hidden">

        {/* Header */}
        <div className="flex items-center justify-between px-4 sm:px-6 py-4 border-b border-zinc-800 bg-zinc-900/50">
          <div className="flex items-center gap-2 sm:gap-3 min-w-0">
            <span className="text-[11px] font-mono text-zinc-400 bg-zinc-950 px-2 py-1 rounded border border-zinc-700 truncate">
              {voucher.code}
            </span>
            <span className="hidden sm:inline-flex items-center gap-1.5 px-2 py-1 border rounded-md text-[10px] font-medium bg-zinc-800/50 border-zinc-700/50 text-zinc-300 truncate">
              {voucherTypeName}
            </span>
          </div>
          <button onClick={onClose} className="text-zinc-500 hover:text-zinc-100 transition-colors flex-shrink-0">
        ...
        <div className="flex flex-col md:flex-row flex-1 overflow-y-auto md:overflow-hidden">
          ...
          {/* Main Details Pane */}
          <div className="flex-1 p-4 sm:p-6 overflow-y-auto border-b md:border-b-0 md:border-r border-zinc-800 bg-zinc-950/30 space-y-6">
            ...
            {duplicateFlagged && voucher.duplicate_flag && (
              <div className="bg-red-500/10 border border-red-500/20 rounded-xl p-4 flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <h4 className="text-sm font-semibold text-red-400 mb-1">System Flag: Potential Duplicate Detected</h4>
                  <p className="text-xs text-red-500/80 leading-relaxed mb-3">
                    {voucher.duplicate_flag.reason}
                  </p>
                  <div className="flex items-center gap-2 flex-wrap">
                    <span className="text-[11px] font-medium text-red-300 bg-red-500/20 px-3 py-1.5 rounded">
                      Match: {voucher.duplicate_flag.match_ref}
                    </span>
                    <button
                      onClick={() => onDismissDuplicate(voucher.id)}
                      disabled={isPending}
                      className="text-[11px] font-medium text-zinc-400 bg-zinc-900 border border-zinc-700 hover:bg-zinc-800 px-3 py-1.5 rounded transition-colors disabled:opacity-40"
                    >
                      Dismiss Flag
                    </button>
                  </div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <div className="inline-flex items-center gap-1.5 px-2 py-1 bg-pri border border-zinc-800 rounded-md text-[10px] text-zinc-400">
            ...
            {fileFields.length > 0 && (
              <div>
                <p className="text-xs font-medium text-zinc-300 mb-3">Supporting Documents</p>
                <div className="space-y-2">
                  {fileFields.map((fv) => (
                    <a
                      ...
                      target="_blank"
                      rel="noopener noreferrer"
                      className="flex items-center gap-3 p-3 rounded-lg border border-zinc-800 bg-zinc-900/30 group hover:border-zinc-600 transition-colors"
                    >
                      <div className="w-10 h-10 rounded bg-zinc-800 flex items-center justify-center text-zinc-300 flex-shrink-0">
          ...
          <div className="w-full md:w-72 bg-zinc-900/30 p-4 sm:p-6 flex flex-col justify-between overflow-y-auto flex-shrink-0">

            {/* Approval History */}
            <div>
              <p className="text-[10px] font-medium text-zinc-500 uppercase tracking-widest mb-4">Approval Chain</p>
              {sortedHistory.length === 0 ? (
                <p className="text-xs text-zinc-500 italic">No approval history yet.</p>
              ) : (
                <div className="relative border-l border-zinc-700 ml-2.5 space-y-5">
                  {sortedHistory.map((entry) => (
                    <div key={entry.id} className="relative pl-5">
                      <div
                        className={`absolute -left-[5px] top-1 w-2.5 h-2.5 rounded-full border-2 border-zinc-900 ${
                      ...
                      <div className="flex flex-col">
                        ...
                        {entry.actor?.name && (
                          <span className="text-[10px] text-zinc-500 mt-0.5">
                            {entry.action === "approved" ? "Approved by" : entry.action === "rejected" ? "Rejected by" : "Actioned by"}{" "}
                            {entry.actor.name}
                          </span>
                        )}
                        {entry.acted_at && (
                        ...
                        {entry.comment && (
                          <span className="text-[10px] text-zinc-400 mt-1 italic">"{entry.comment}"</span>
                        )}
                        {entry.action === "pending" && (
                          <span className="text-[10px] text-zinc-400 mt-0.5">Awaiting review</span>
                        )}
                      </div>
                    </div>
                  ))}
            ...
            {isActionable ? (
                ...
                {!rejectMode ? (
                  <div className="flex flex-col gap-3">
                    <button
                      ...
                      disabled={isPending}
                      className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-zinc-100 text-zinc-900 hover:bg-white text-sm font-medium transition-all shadow-sm disabled:opacity-40 disabled:cursor-not-allowed"
                    >
                    ...
                    <button
                      ...
                      disabled={isPending}
                      className="w-full flex items-center justify-center gap-2 py-2.5 rounded-lg bg-zinc-900 border border-zinc-700 text-red-400 hover:bg-red-500/10 hover:border-red-500/30 text-sm font-medium transition-all disabled:opacity-40"
                    >
                  ...
                  <div className="space-y-3">
                    <label className="block text-[10px] font-medium text-zinc-400 uppercase tracking-widest">
                      Rejection Reason
                    </label>
                    <textarea
                      autoFocus
                      ...
                      className="w-full bg-zinc-950 border border-zinc-700 rounded-lg p-2.5 text-xs text-zinc-200 placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-zinc-600 resize-none h-20 transition-all"
                    />
                    <div className="flex gap-2">
                      <button
                        onClick={() => setRejectMode(false)}
                        disabled={isPending}
                        className="flex-1 py-2 rounded-lg bg-zinc-800 text-zinc-300 text-xs font-medium hover:bg-zinc-700 transition-colors disabled:opacity-40"
                      >
                        Cancel
                      </button>
                      <button
              ...
              <div className="pt-6 mt-6 border-t border-zinc-800">
                <div className="text-center py-3 rounded-lg bg-zinc-800/50 border border-zinc-700">
...
function VoucherRow({ voucher, onClick }: { voucher: Voucher; onClick: () => void }) {
  ...
  return (
    <div
      onClick={onClick}
      className="group p-3 sm:p-4 hover:bg-zinc-800/40 transition-colors cursor-pointer flex flex-col sm:flex-row sm:items-center gap-3 sm:gap-4 relative"
    >
      {/* Status bar */}
      <div
        className={`absolute left-0 top-0 bottom-0 w-1 hidden sm:block ${
          voucher.status === "pending" && flagged ? "bg-red-500" : voucher.status === "pending" ? "bg-zinc-600" : "bg-transparent"
        }`}
      />

      <div className="flex items-center gap-3 sm:gap-4 flex-1 min-w-0 sm:pl-2">
        <div className="w-9 h-9 sm:w-10 sm:h-10 rounded-xl flex items-center justify-center flex-shrink-0 bg-zinc-800/50 text-zinc-400 border border-zinc-700/50 text-xs font-semibold">
        ...
        <div className="min-w-0">
          <div className="flex items-center gap-2 mb-1">
            ...
            {flagged && (
              <div className="flex items-center justify-center w-5 h-5 rounded bg-red-500/10 text-red-400 flex-shrink-0">
                <Warning2 size={12} variant="Bulk" color="currentColor" />
              </div>
            )}
          </div>
          <div className="flex items-center gap-2 text-[11px] flex-wrap">
            <span className="font-mono text-zinc-400 bg-zinc-900 px-1.5 py-0.5 rounded border border-zinc-800 tracking-wide">
              {voucher.code}
            </span>
            <span className="text-zinc-500">•</span>
            <span className="text-zinc-400 truncate">{voucher.department?.name ?? "—"}</span>
          </div>
        </div>
      </div>

      <div className="flex items-center justify-between sm:justify-end gap-6 sm:gap-10 flex-shrink-0 pl-12 sm:pl-0">
        <div className="text-left sm:text-right hidden md:block">
          <p className="text-[10px] text-zinc-500 mb-0.5">Submitted</p>
          <p className="text-xs font-medium text-zinc-300">{formatDate(voucher.created_at)}</p>
        </div>
        <div className="text-left sm:text-right">
          <p className="text-[10px] text-zinc-500 mb-0.5">{amountRaw ? "Amount" : "Name"}</p>
          <p className="text-sm font-semibold text-zinc-100">{displayLabel}</p>
        </div>
        <div className="text-zinc-600 group-hover:text-zinc-300 transition-colors hidden sm:block">
...
export default function ApprovalInboxPage() {
  ...
  const handleApprove = (id: string) => {
    approveMutation.mutate({ id }, { onSuccess: (updated) => setSelectedVoucher(updated) });
  };

  const handleReject = (id: string, reason: string) => {
    rejectMutation.mutate({ id, payload: { reason } }, { onSuccess: (updated) => setSelectedVoucher(updated) });
  };
        ...
        <div className="border-b border-zinc-800/80 bg-zinc-950/80 backdrop-blur-md sticky top-0 z-10">
          <div className="px-4 sm:px-6 py-3 sm:py-0 sm:h-16 flex flex-col sm:flex-row sm:items-center justify-between gap-3 max-w-7xl mx-auto">
            <div className="flex items-center gap-3 flex-wrap min-w-0">
              ...
              <h1 className="text-sm font-medium text-zinc-100 truncate">Approval Inbox</h1>
            ...
            <div className="flex items-center gap-2 text-xs text-zinc-500 font-medium">
              ...
              <span className="truncate">Acting as: {MY_ROLE}</span>
            </div>
          </div>
        </div>
        ...
        <div className="px-4 sm:px-6 py-6 sm:py-8 max-w-7xl mx-auto space-y-8 sm:space-y-10">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-5">
            ...
            <StatCard
              ...
              value={duplicateFlagsCount}
              sub="System warnings detected"
              alert={duplicateFlagsCount > 0}
            />
            <StatCard label="Processed Today" value={processedTodayCount} sub="Approved or rejected" />
            <StatCard
              label="Voucher Types"
              value={new Set(allVouchers.map((v) => v.voucher_type_id)).size}
              sub="Across all submissions"
            />
          </div>

          <div className="bg-zinc-900/40 border border-zinc-800/50 rounded-xl overflow-hidden shadow-sm">

            {/* Tabs */}
            <div className="border-b border-zinc-800/80 bg-zinc-900/50">
              <div className="flex items-center gap-4 sm:gap-6 px-3 sm:px-4 pt-4 border-b border-zinc-800 overflow-x-auto">
                {TAB_CONFIG.map(({ key, label, badge }) => {
                  ...
                  return (
                    <button
                      ...
                      className={`pb-3 text-sm font-medium transition-colors border-b-2 whitespace-nowrap flex items-center gap-1.5 flex-shrink-0 ${
                        isActive
                          ? isFlagged
                      ...
                      {badge !== undefined && badge > 0 && (
                        <span
                          className={`py-0.5 px-1.5 rounded text-[10px] ${
                            isFlagged ? "bg-red-500/20 text-red-400" : "bg-zinc-800 text-zinc-400"
                          }`}
                        >
                          {badge}
                        </span>
              ...
              <div className="p-3 sm:p-4">
                <div className="relative w-full sm:w-80">
                  ...
                  <input
                    ...
                    className="w-full bg-zinc-950 border border-zinc-800 rounded-lg pl-10 pr-4 py-2 text-sm text-zinc-200 placeholder-zinc-600 focus:outline-none focus:ring-1 focus:ring-zinc-600 transition-all"
                  />
                </div>
              </div>
            </div>
            ...
            {isLoading ? (
              ...
              <div className="py-16 sm:py-24 flex flex-col items-center justify-center text-center px-4">
                <Filter size={32} className="text-zinc-700 mb-4" color="currentColor" />
                <p className="text-sm font-medium text-zinc-300">No records found</p>
